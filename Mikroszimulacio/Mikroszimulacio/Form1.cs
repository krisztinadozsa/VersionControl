...
namespace Mikroszimulacio
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            Population = GetPopulation(@"E:\Mikroszimuláció\nép.csv");
            BirthProbabilities = GetBirthProbabilities(@"E:\Mikroszimuláció\születés.csv");
            DeathProbabilities = GetDeathProbabilities(@"E:\Mikroszimuláció\halál.csv");
            ...
            for (int year = 2005; year <= 2024; year++)
            {
                ...
                Console.WriteLine(string.Format("Év: {0} Ferfiak: {1} Nők: {2}", year, ferfiakszama, nokszama));
            }
        ...
        private void SzimulaciosLepes(Person person, int year)
        {
            ...
            if (person.IsActive && person.Gender == Gender.Female)
            {
        ...
        public List<BirthProbabilities> GetBirthProbabilities(string csvpath)
        {
            ...
            using (StreamReader sr = new StreamReader(csvpath, Encoding.Default))
            {
                while (!sr.EndOfStream)
                {
                    ...
                    p.NrbOfChildren = int.Parse(items[1]);
